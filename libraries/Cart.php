...
class Cart
{
    ...
    public function __construct($params = array())
    {
        ...
        \CI\Core\log_message('debug', "Cart Class Initialized");
    }
    ...
    public function insert($items = array())
    {
        ...
        if (!is_array($items) || count($items) == 0) {
            \CI\Core\log_message('error', 'The insert method must be passed an array containing data.');
            return false;
        }
        ...
        $save_cart = false;
        if (isset($items['id'])) {
            if (($rowid = $this->inserItem($items))) {
                $save_cart = true;
            }
        } else {
            foreach ($items as $val) {
                if (is_array($val) && isset($val['id'])) {
                    if ($this->inserItem($val)) {
                        $save_cart = true;
                    }
                }
            }
     ...
     * @return  bool
     */
    private function inserItem($items = array())
    {
        // Was any cart data passed? No? Bah...
        if (!is_array($items) || count($items) == 0) {
            \CI\Core\log_message('error', 'The insert method must be passed an array containing data.');
        ...
        if (!isset($items['id']) || !isset($items['qty']) || !isset($items['price']) || !isset($items['name'])) {
            \CI\Core\log_message('error', 'The cart array must contain a product ID, quantity, price, and name.');
            return false;
        }
        ...
        if (!preg_match("/^[" . $this->product_name_rules . "]+$/i", $items['name'])) {
            \CI\Core\log_message(
                'error',
                'An invalid name was submitted as the product name: ' . $items['name'] .
        ...
        if (!is_numeric($items['price'])) {
            \CI\Core\log_message('error', 'An invalid price was submitted for product ID: ' . $items['id']);
            return false;
        }
    ...
    public function update($items = array())
    {
        ...
        if (isset($items['rowid']) && isset($items['qty'])) {
            if ($this->updateItem($items) == true) {
                $save_cart = true;
            }
        } else {
            foreach ($items as $val) {
                if (is_array($val) && isset($val['rowid']) && isset($val['qty'])) {
                    if ($this->updateItem($val) == true) {
                        $save_cart = true;
                    }
                }
            }
     ...
     * @return  bool
     */
    private function updateItem($items = array())
    {
    ...
    private function saveCart()
    {
        ...
        foreach ($this->cart_contents as $key => $val) {
            ...
            $this->cart_contents[$key]['subtotal'] =
                ($this->cart_contents[$key]['price'] * $this->cart_contents[$key]['qty']);
        }
    ...
    {
        if (!isset($this->cart_contents[$rowid]['options']) || count($this->cart_contents[$rowid]['options']) === 0) {
            return false;
        }
